...
function renderAll(){
  setTimeout(renderHome, 50); // DOM 렌더링 후 실행
...
function renderCurrent(){
  ...
  if(curPage==='home') setTimeout(renderHome, 50);
...
function renderHome(){
  ...
  // 일정 카드 (schedule 권한 있을 때만)
  if(isAdmin || tabs.includes('schedule')){
    ...
    var sh = upcoming.length ? upcoming.map(function(s){
    ...
    }).join('') : '<div style="text-align:center;padding:12px;color:var(--text3);font-size:13px">예정된 일정이 없어요</div>';
    cards.push({title:'📅 오늘·예정 일정', btn:'일정 탭 →', btnFn:"switchPage('schedule')", body:sh});
  }

  // 부족품목 카드 (inv 권한 있을 때만)
  if(isAdmin || tabs.includes('inv')){
    var lows = db.items.filter(function(it){return it.min>0&&it.qty<=it.min;});
    var lh = lows.length ? lows.map(function(it){
      return '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 4px;border-bottom:1px solid var(--border);font-size:12px">'
    ...
    }).join('') : '<div style="text-align:center;padding:12px;color:var(--green);font-size:13px">✓ 모든 품목 재고 정상</div>';
    cards.push({title:'⚠️ 부족 품목', btn:'재고 →', btnFn:"switchPage('inv')", body:lh});
  }

  // 점검현황 카드 (jongham 또는 seomigam 권한 있을 때만)
  if(isAdmin || tabs.includes('jongham') || tabs.includes('seomigam')){
    var curFY2 = getCurrentFY();
    var ih = '';
    var bldKeys = [];
    if(isAdmin||tabs.includes('jongham')) bldKeys.push('jongham');
    if(isAdmin||tabs.includes('seomigam')) bldKeys.push('seomigam');
    ...
    var inspBtns = '';
    if(isAdmin||tabs.includes('jongham')) inspBtns += '<button class="btn btn-xs" onclick="switchPage(\'jongham\')">종합관 →</button>';
    if(isAdmin||tabs.includes('seomigam')) inspBtns += '<button class="btn btn-xs" onclick="switchPage(\'seomigam\')" style="margin-left:4px">서미감관 →</button>';
    cards.push({title:'🔍 점검 현황', customBtn:inspBtns, body:ih});
  }

  // 업무일지 카드 (journal 권한 있을 때만)
  if(isAdmin || tabs.includes('journal')){
    var recent = [...db.journals].sort(function(a,b){return b.createdAt.localeCompare(a.createdAt);}).slice(0,3);
    var jh = recent.length ? recent.map(function(j){
      return '<div style="padding:8px 4px;border-bottom:1px solid var(--border);cursor:pointer" onclick="switchPage(\'journal\')">'
    ...
    }).join('') : '<div style="text-align:center;padding:12px;color:var(--text3);font-size:13px">최근 업무일지가 없어요</div>';
    cards.push({title:'📝 최근 업무일지', btn:'업무일지 탭 →', btnFn:"switchPage('journal')", body:jh});
  }

  // 카드 없으면
  if(!cards.length){
    homeBody.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text3)">접근 가능한 탭이 없어요<br>관리자에게 문의하세요</div>';
    return;
  }

  // 2열 그리드로 렌더링
  var gridHtml = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px">';
  cards.forEach(function(card){
    var btnHtml = card.customBtn || (card.btn ? '<button class="btn btn-xs" onclick="'+card.btnFn+'">'+card.btn+'</button>' : '');
    gridHtml += '<div style="background:var(--surface);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden">'
      ...
      +'<div style="font-size:13px;font-weight:700;color:var(--text)">'+card.title+'</div>'
      +'<div style="display:flex;gap:4px">'+btnHtml+'</div></div>'
      +'<div style="padding:10px">'+card.body+'</div>'
      +'</div>';
  });
  gridHtml += '</div>';
  homeBody.innerHTML = gridHtml;
}
