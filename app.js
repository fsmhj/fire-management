...
function doLogin(){
  ...
  function checkLogin(userPw, adminPw){
    ...
    if(pw===adminPw){
      isAdmin=true;userName=name;myTabs=ALL_TABS.map(function(t){return t.id;});enterApp();
    } else if(pw===userPw){
      isAdmin=false;userName=name;
      const member=db.members.find(function(m){return m.name===name;});
...
function tryAutoLogin(){
  ...
  if(n&&t){
    ...
    // Firebase에서 최신 팀원 권한 가져오기
    if(!isAdmin && typeof firebase!=='undefined' && firebase.apps&&firebase.apps.length){
      firebase.database().ref('sobangtm/members').once('value').then(function(snap){
        const members=snap.val()||[];
        const member=Array.isArray(members)?members.find(function(m){return m.name===n;}):Object.values(members).find(function(m){return m.name===n;});
        myTabs=member?member.tabs:JSON.parse(t);
        enterApp();
      }).catch(function(){
        myTabs=JSON.parse(t);
        enterApp();
...
function checkKicked(){
  ...
  const kicked = db.kickedUsers||[];
  if(kicked.includes(userName)){
    alert('관리자에 의해 접근이 차단됐습니다. 로그아웃됩니다.');
...
function deleteMember(id){
  ...
  // 강제로그아웃 목록에 추가
  if(member){
    db.kickedUsers = db.kickedUsers||[];
    if(!db.kickedUsers.includes(member.name)) db.kickedUsers.push(member.name);
  }
