...
function renderExtinguisher(){
  ...
  const replace=bldData.filter(it=>it.year&&(curYear-it.year)>=10).length;
  const warn=bldData.filter(it=>it.year&&(curYear-it.year)===9).length;

  // 건물 탭 버튼
  let bldTabs='<div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:6px;margin-bottom:12px;-ms-overflow-style:none;scrollbar-width:none">';
  EXT_BUILDINGS.forEach(function(b){
    const isActive=b===curExtBuilding;
    const bData=db.extinguishers[b]||[];
    const bReplace=bData.filter(it=>it.year&&(curYear-it.year)>=10).length;
  ...
  // 검색+추가 버튼
  let toolbar='<div style="display:flex;gap:8px;margin-bottom:12px;align-items:center">';
  toolbar+='<input type="text" id="extSearch" placeholder="🔍 층·실명 검색..." oninput="renderExtList()" style="flex:1">';
...
function renderExtList(){
  ...
  items.forEach(function(it){
    ...
    const isReplace=age!==null&&age>=10;
    const isWarn=age!==null&&age===9;
    const rowBg=isReplace?'rgba(220,38,38,0.08)':isWarn?'rgba(217,119,6,0.08)':'';
    const statusBadge=isReplace?'<span class="badge b-red">⚠ 교체대상</span>':isWarn?'<span class="badge b-amber">교체임박</span>':'<span class="badge b-green">정상</span>';
