...
function renderAll(){
  ...
  if(document.getElementById('todoContent'))renderTodo();
  if(document.getElementById('facilityContent'))renderFacility();
...
function renderCurrent(){
  ...
  if(curPage==='todo')renderTodo();
  if(curPage==='facility')renderFacility();
...
function renderExtList(){
  ...
  items.forEach(function(it, idx){
    ...
    html+='<td style="padding:7px 6px;color:var(--text3)">'+(idx+1)+'</td>';
...
// ── 할일 체크리스트 ──
let curTodoDate = '';
let curTodoView = 'today'; // 'today' | 'history'

function renderTodo(){
  const el = document.getElementById('todoContent');
  if(!el) return;
  if(!curTodoDate) curTodoDate = tod();
  const curYear = new Date().getFullYear();
  const fy = getCurrentFY();

  // 오늘 날짜의 할일 (개인 + 팀공유)
  const todos = (db.todos||[]).filter(function(t){
    if(curTodoView==='today') return t.date===curTodoDate;
    // 회계연도 전체
    const r = getFYRange(fy);
    return t.date>=r.start && t.date<=r.end;
  });
  const myTodos = todos.filter(function(t){ return t.share==='private'&&t.by===userName; });
  const sharedTodos = todos.filter(function(t){ return t.share==='shared'; });
  ...
  html += '<button class="btn btn-sm" onclick="moveTodoDate(-1)">‹</button>';
  html += '<input type="date" value="'+curTodoDate+'" onchange="curTodoDate=this.value;renderTodo()" style="border:1.5px solid var(--border2);border-radius:var(--radius-sm);padding:6px 10px;font-size:13px;font-family:inherit;background:var(--surface);color:var(--text)">';
  html += '<button class="btn btn-sm" onclick="moveTodoDate(1)">›</button>';
  html += '<button class="btn btn-sm" onclick="curTodoDate=tod();renderTodo()" style="font-size:11px">오늘</button>';
  html += '</div>';
  html += '<div style="display:flex;gap:6px">';
  html += '<button class="btn btn-sm'+(curTodoView==='today'?' btn-primary':'')+'" onclick="curTodoView=\'today\';renderTodo()">📅 날짜별</button>';
  html += '<button class="btn btn-sm'+(curTodoView==='history'?' btn-primary':'')+'" onclick="curTodoView=\'history\';renderTodo()">📋 전체이력</button>';
  html += '</div>';
  html += '</div>';
  ...
  if(total>0){
    ...
    html += '<div style="background:var(--surface);border-radius:var(--radius-sm);padding:12px 14px;margin-bottom:12px;box-shadow:var(--shadow)">';
    html += '<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:6px"><span style="color:var(--text2)">오늘 진행률</span><span style="font-weight:700;color:var(--primary)">'+done+'/'+total+' ('+pct+'%)</span></div>';
    html += '<div style="background:var(--border);border-radius:4px;height:8px"><div style="background:var(--primary);border-radius:4px;height:8px;width:'+pct+'%;transition:width .3s"></div></div>';
    html += '</div>';
  }

  // 추가 버튼
  html += '<button class="btn btn-primary btn-sm" onclick="openAddTodo()" style="width:100%;justify-content:center;margin-bottom:14px">+ 할일 추가</button>';

  // 개인 할일
  if(myTodos.length || curTodoView==='today'){
    html += '<div style="font-size:12px;font-weight:700;color:var(--text2);margin-bottom:8px">🔒 개인 할일</div>';
    if(!myTodos.length){
      html += '<div style="text-align:center;padding:16px;color:var(--text3);font-size:13px;background:var(--surface);border-radius:var(--radius-sm);margin-bottom:12px">등록된 할일이 없어요</div>';
    } else {
      html += renderTodoList(myTodos);
    }
  }

  // 팀 공유 할일
  html += '<div style="font-size:12px;font-weight:700;color:var(--text2);margin:12px 0 8px">👥 팀 공유 할일</div>';
  if(!sharedTodos.length){
    html += '<div style="text-align:center;padding:16px;color:var(--text3);font-size:13px;background:var(--surface);border-radius:var(--radius-sm)">등록된 팀 할일이 없어요</div>';
  } else {
    html += renderTodoList(sharedTodos);
  }
...
function renderTodoList(list){
  let html = '';
  list.sort(function(a,b){ return (a.done?1:0)-(b.done?1:0)||(a.createdAt||'').localeCompare(b.createdAt||''); });
  list.forEach(function(t){
    const isDone = t.done;
    html += '<div style="display:flex;align-items:flex-start;gap:10px;padding:12px 14px;background:var(--surface);border-radius:var(--radius-sm);margin-bottom:8px;box-shadow:var(--shadow);opacity:'+(isDone?.6:1)+';border-left:3px solid '+(isDone?'var(--green)':'var(--primary)')+'">';
    html += '<button onclick="toggleTodoDone(\''+t.id+'\')" style="width:24px;height:24px;border-radius:50%;border:2px solid '+(isDone?'var(--green)':'var(--border2)')+';background:'+(isDone?'var(--green)':'transparent')+';cursor:pointer;flex-shrink:0;margin-top:1px;display:flex;align-items:center;justify-content:center;font-size:13px">'+(isDone?'✓':'')+'</button>';
    html += '<div style="flex:1;min-width:0">';
    html += '<div style="font-size:13px;color:var(--text);line-height:1.6;'+(isDone?'text-decoration:line-through;color:var(--text3)':'')+';white-space:pre-line">'+t.content+'</div>';
    html += '<div style="font-size:11px;color:var(--text3);margin-top:4px">'+t.by+' · '+t.date+(t.share==='shared'?' · 팀공유':'')+'</div>';
    html += '</div>';
    html += '<div style="display:flex;gap:4px;flex-shrink:0">';
    if(t.by===userName||isAdmin){
      html += '<button class="btn btn-xs" onclick="openEditTodo(\''+t.id+'\')">수정</button>';
      html += '<button class="btn btn-xs btn-outline-red" onclick="deleteTodo(\''+t.id+'\')">삭제</button>';
    }
    html += '</div></div>';
  });
  return html;
...
function moveTodoDate(d){
  const dt = new Date(curTodoDate);
  dt.setDate(dt.getDate()+d);
  curTodoDate = dt.toISOString().slice(0,10);
  renderTodo();
}

function setTodoShare(v){
  document.getElementById('todo_share').value=v;
  document.getElementById('todo_priv_btn').style.background=v==='private'?'var(--primary)':'';
  document.getElementById('todo_priv_btn').style.color=v==='private'?'#fff':'';
  document.getElementById('todo_share_btn').style.background=v==='shared'?'var(--primary)':'';
  document.getElementById('todo_share_btn').style.color=v==='shared'?'#fff':'';
}

function openAddTodo(){
  document.getElementById('todo_id').value='';
  document.getElementById('todoModalTitle').textContent='할일 추가';
  document.getElementById('todo_date').value=curTodoDate||tod();
  document.getElementById('todo_content').value='';
  setTodoShare('private');
  openModal('addTodo');
}

function openEditTodo(id){
  const t=(db.todos||[]).find(function(x){return x.id===id;});
  if(!t) return;
  document.getElementById('todo_id').value=id;
  document.getElementById('todoModalTitle').textContent='할일 수정';
  document.getElementById('todo_date').value=t.date;
  document.getElementById('todo_content').value=t.content;
  setTodoShare(t.share||'private');
  openModal('addTodo');
}
...
function saveTodo(){
  ...
  const content=document.getElementById('todo_content').value.trim();
  const share=document.getElementById('todo_share').value||'private';
  if(!content){toast('할일 내용을 입력해 주세요','error');return;}
  db.todos=db.todos||[];
  if(id){
    const t=db.todos.find(function(x){return x.id===id;});
    if(t) Object.assign(t,{date,content,share,updatedAt:tod()});
  } else {
    db.todos.push({id:uid(),date,content,share,done:false,createdAt:new Date().toISOString(),by:getW()});
  }
  saveLocal();closeModal('addTodo');renderTodo();
  toast(id?'수정됐습니다':'추가됐습니다','success');
}
...
function toggleTodoDone(id){
  ...
  if(!t) return;
  t.done=!t.done;
  t.doneAt=t.done?tod():null;
  saveLocal();renderTodo();
}

function deleteTodo(id){
  if(!confirm('삭제하시겠습니까?')) return;
  db.todos=(db.todos||[]).filter(function(x){return x.id!==id;});
  saveLocal();renderTodo();toast('삭제됐습니다');
}

// ── 소방시설 위치기록 ──
const FAC_BUILDINGS = ['신관','응급센터','외상센터','외래센터','후생관','정문주차장(후생관)','종합관','의학관','별관','진리관','영빈관','루가홀','원의1학사','원의2학사','원의3학사','장례식장','장례식장 철골주차장','부설주차장 및 재활용창고','가설사무실','YWCA 건물'];
const FAC_TYPES = ['소화전','방수기구함','셔터연동제어기','슈퍼비조리판넬','수신기','중계기','스프링클러헤드','감지기','기타'];
let curFacBuilding = FAC_BUILDINGS[0];

function renderFacility(){
  const el=document.getElementById('facilityContent');
  if(!el) return;
  const facData=(db.facilities||{})[curFacBuilding]||[];

  // 건물 탭
  let bldTabs='<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">';
  FAC_BUILDINGS.forEach(function(b){
    const cnt=((db.facilities||{})[b]||[]).length;
    const isActive=b===curFacBuilding;
    bldTabs+='<button onclick="switchFacBuilding(\''+b+'\')" style="padding:6px 12px;border-radius:20px;border:1.5px solid '+(isActive?'var(--primary)':'var(--border2)')+';background:'+(isActive?'var(--primary)':'var(--surface)')+';color:'+(isActive?'#fff':'var(--text2)')+';font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;font-family:inherit">'+b+(cnt?'<span style="background:rgba(255,255,255,.3);border-radius:10px;font-size:10px;padding:1px 5px;margin-left:4px">'+cnt+'</span>':'')+'</button>';
  });
  bldTabs+='</div>';

  // 종류 필터 + 검색 + 추가
  let toolbar='<div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;align-items:center">';
  toolbar+='<input type="text" id="facSearch" placeholder="🔍 층/실명/중계기 검색..." oninput="renderFacList()" style="flex:1;min-width:150px">';
  toolbar+='<select id="facTypeFilter" onchange="renderFacList()" style="border:1.5px solid var(--border2);border-radius:var(--radius-sm);padding:8px 10px;font-size:13px;background:var(--surface);color:var(--text);font-family:inherit"><option value="">전체 종류</option>'+FAC_TYPES.map(function(t){return'<option value="'+t+'">'+t+'</option>';}).join('')+'</select>';
  toolbar+='<button class="btn btn-primary btn-sm" onclick="openAddFacility()">+ 추가</button>';
  toolbar+='</div>';

  el.innerHTML=bldTabs+toolbar+'<div id="facList"></div>';
  renderFacList();
}

function renderFacList(){
  const el=document.getElementById('facList'); if(!el) return;
  const q=(document.getElementById('facSearch')||{}).value||'';
  const typeF=(document.getElementById('facTypeFilter')||{}).value||'';
  let items=((db.facilities||{})[curFacBuilding]||[]).filter(function(f){
    if(typeF&&f.type!==typeF) return false;
    if(!q) return true;
    return (f.floor+f.type+(f.relay||'')+(f.memo||'')).toLowerCase().includes(q.toLowerCase());
  });

  if(!items.length){
    el.innerHTML='<div class="empty"><div class="empty-icon">🗺️</div><div>등록된 시설이 없어요<br>+ 추가 버튼으로 등록해 주세요</div></div>';
    return;
  }

  const typeColor={'소화전':'b-primary','방수기구함':'b-teal','셔터연동제어기':'b-amber','슈퍼비조리판넬':'b-purple','수신기':'b-green','중계기':'b-gray','스프링클러헤드':'b-primary','감지기':'b-amber','기타':'b-gray'};
  let html='';
  items.forEach(function(f){
    html+='<div style="background:var(--surface);border-radius:var(--radius-sm);box-shadow:var(--shadow);padding:12px 14px;margin-bottom:8px;border-left:3px solid var(--primary)">';
    ...
    html+='<span style="font-size:14px;font-weight:700;color:var(--text)">'+f.floor+'</span>';
    html+='<span class="badge '+(typeColor[f.type]||'b-gray')+'">'+f.type+'</span>';
    if(f.relay) html+='<span style="font-size:12px;color:var(--text2)">중계기: <strong>'+f.relay+'</strong></span>';
    html+='</div>';
    if(f.has45a||f.has65a){
      html+='<div style="font-size:12px;color:var(--text2);margin-bottom:4px">방수구: ';
      if(f.has45a) html+='<span class="badge b-primary" style="font-size:10px;margin-right:4px">45A</span>';
      if(f.has65a) html+='<span class="badge b-teal" style="font-size:10px">65A</span>';
      html+='</div>';
    }
    if(f.memo) html+='<div style="font-size:12px;color:var(--text3)">'+f.memo+'</div>';
    html+='</div>';
    html+='<div style="display:flex;gap:4px;flex-shrink:0">';
    html+='<button class="btn btn-xs" onclick="openEditFacility(\''+f.id+'\')">수정</button>';
    html+='<button class="btn btn-xs btn-outline-red" onclick="deleteFacility(\''+f.id+'\')">삭제</button>';
    html+='</div></div></div>';
  });
  el.innerHTML=html;
}

function switchFacBuilding(b){
  curFacBuilding=b;
  renderFacility();
}
...
function openAddFacility(){
  ...
  document.getElementById('fac_bld').value=curFacBuilding;
  document.getElementById('facilityModalTitle').textContent=curFacBuilding+' 시설 위치 등록';
  ['fac_floor','fac_relay','fac_memo'].forEach(function(id){document.getElementById(id).value='';});
  document.getElementById('fac_type').value='소화전';
  document.getElementById('fac_45a').checked=false;
  document.getElementById('fac_65a').checked=false;
  openModal('addFacility');
}

function openEditFacility(id){
  const all=Object.values(db.facilities||{}).flat();
  const f=all.find(function(x){return x.id===id;}); if(!f) return;
  document.getElementById('fac_id').value=id;
  document.getElementById('fac_bld').value=f.bld;
  document.getElementById('facilityModalTitle').textContent='시설 위치 수정';
  document.getElementById('fac_floor').value=f.floor||'';
  document.getElementById('fac_type').value=f.type||'소화전';
  document.getElementById('fac_relay').value=f.relay||'';
  document.getElementById('fac_45a').checked=!!f.has45a;
  document.getElementById('fac_65a').checked=!!f.has65a;
  document.getElementById('fac_memo').value=f.memo||'';
  openModal('addFacility');
}

function saveFacility(){
  const id=document.getElementById('fac_id').value;
  const bld=document.getElementById('fac_bld').value;
  const floor=document.getElementById('fac_floor').value.trim();
  const type=document.getElementById('fac_type').value;
  const relay=document.getElementById('fac_relay').value.trim();
  const has45a=document.getElementById('fac_45a').checked;
  const has65a=document.getElementById('fac_65a').checked;
  const memo=document.getElementById('fac_memo').value.trim();
  if(!floor){toast('층/위치를 입력해 주세요','error');return;}
  db.facilities=db.facilities||{};
  if(!db.facilities[bld]) db.facilities[bld]=[];
  if(id){
    const f=db.facilities[bld].find(function(x){return x.id===id;});
    if(f) Object.assign(f,{floor,type,relay,has45a,has65a,memo,bld});
  } else {
    db.facilities[bld].push({id:uid(),bld,floor,type,relay,has45a,has65a,memo,by:getW(),createdAt:tod()});
  }
  saveLocal();closeModal('addFacility');renderFacility();
  toast(id?'수정됐습니다':'등록됐습니다','success');
}

function deleteFacility(id){
  if(!confirm('삭제하시겠습니까?')) return;
  Object.keys(db.facilities||{}).forEach(function(bld){
    db.facilities[bld]=(db.facilities[bld]||[]).filter(function(x){return x.id!==id;});
  });
  saveLocal();renderFacility();toast('삭제됐습니다');
}
