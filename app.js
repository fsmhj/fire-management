...
function doLogin(){
  ...
  function checkLogin(userPw, adminPw){
    ...
    if(pw===adminPw){
      ...
      myTabs=member?member.tabs:['home','inv','journal','schedule','memo','jongham','seomigam'];
      enterApp();
    } else {
...
function switchPage(p){
  if(p==='txnHistory'){curPage=p;document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.id==='page-'+p));document.getElementById('fabBtn').style.display='none';renderTxnHistory();return;}
  ...
  const hideFab=['settings','home','schedule'];
  document.getElementById('fabBtn').style.display=hideFab.includes(p)?'none':'flex';
  renderCurrent();
}
function renderAll(){
...
function fabAction(){
  ...
  if(curPage==='inv')openAddItem();
  if(curPage==='journal')openAddJournal();
  if(curPage==='schedule')openAddSchedule();
  if(curPage==='memo')openAddMemo();
}
...
function renderHome(){
  ...
  if(schEl){
    ...
    if(!upcoming.length){
      schEl.innerHTML='<div style="text-align:center;padding:12px;color:var(--green);font-size:13px">예정된 일정이 없어요</div>';
    } else {
      var sh='';
  ...
  if(inspEl){
    ...
    var ih='';
    ['jongham','seomigam'].forEach(function(bldKey){
...
function renderBuilding(bldKey){
  ...
  fys.forEach(function(fy){
    ...
    if(fyInsps.length){
      ...
      html += '<div style="text-align:center;padding:20px;color:var(--text3);font-size:13px">등록된 점검 이력이 없어요<br>';
      html += '<button class="btn btn-primary btn-sm" style="margin-top:8px" onclick="openAddInspection(\'' + bldKey + '\')">+ 점검 등록</button></div>';
    }
...
function openAddInspection(bldKey){
  ...
  document.getElementById('insp_repair').value='';
...
function renderRemedyList(inspId){
  const rems = (db.remedies||[]).filter(r=>r.inspId===inspId);
  if(!rems.length) return '<div style="font-size:12px;color:var(--text3);padding:4px 0">등록된 보완사항이 없어요</div>';
  const total = rems.length;
  const done  = rems.filter(r=>r.status==='완료').length;
  return `
    <div style="font-size:11px;color:var(--text3);margin-bottom:6px">
      전체 ${total}건 · 완료 ${done}건 · 미완료 ${total-done}건
    </div>
    ${rems.map(r=>{
      const st = REMEDY_STATUS[r.status]||{color:'var(--gray)',badge:'b-gray',icon:'📌'};
      return `<div style="display:flex;align-items:flex-start;gap:8px;padding:7px 8px;background:var(--surface);border-radius:var(--radius-xs);margin-bottom:5px;border-left:3px solid ${st.color}">
        <div style="flex:1;min-width:0">
          <div style="font-size:12px;color:var(--text);line-height:1.5">${r.content}</div>
          ${r.memo?`<div style="font-size:11px;color:var(--text3);margin-top:2px">${r.memo}</div>`:''}
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0">
          <span class="badge ${st.badge}" style="font-size:10px">${st.icon} ${r.status||'미처리'}</span>
          <div style="display:flex;gap:3px">
            <button class="btn btn-xs" onclick="openEditRemedy('${r.id}')" style="font-size:10px;padding:2px 6px">수정</button>
            <button class="btn btn-xs btn-outline-red" onclick="deleteRemedy('${r.id}','${inspId}')" style="font-size:10px;padding:2px 6px">삭제</button>
          </div>
        </div>
      </div>`;
    }).join('')}`;
